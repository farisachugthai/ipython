...
class HistoryAccessor(HistoryAccessorBase):
    ...
    def search(
        ...
        """Search the database using unix glob-style matching.
...
class HistoryManager(HistoryAccessor):
    ...
    # necessary.
    _i00 = Unicode("").tag(config=False)
    _i = Unicode("").tag(config=False)
    _ii = Unicode("").tag(config=False)
    _iii = Unicode("").tag(config=False)
    ...
    def _get_range_session(self, start=1, stop=None, raw=True, output=False):
        ...
        Called by get_range, and takes similar parameters.
