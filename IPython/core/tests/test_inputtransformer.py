...
syntax = \
    dict(assign_system=[(i, py3compat.u_format(o)) for i, o in
                        [(u'a =! ls', "a = get_ipython().getoutput('ls')"),
                         ...
                         (u"a,= !echo 'a'",
                          u"a, = get_ipython().getoutput(\"echo 'a'\")"),
                         (u"a, *bc = !echo 'a\\nb\\nc'",
                            u"a, *bc = get_ipython().getoutput(\"echo 'a\\\\nb\\\\nc'\")"),
                         # Tuple unpacking with regular Python expressions, not
                         # our syntax.
         ...
         assign_magic=[(i, py3compat.u_format(o)) for i, o in \
                       [(u'a =% who', "a = get_ipython().run_line_magic('who', '')"),
                        ...
                        ('    ', '    '),  # blank lines are kept intact
                        (u"a, b = %foo",
                         u"a, b = get_ipython().run_line_magic('foo', '')"),
                        ]],

         classic_prompt=[('>>> x=1', 'x=1'),
                         ('x=1', 'x=1'),  # normal input is unmodified
                         ('    ', '    '),  # blank lines are kept intact
                         ],

         ipy_prompt=[('In [1]: x=1', 'x=1'),
                     ('x=1', 'x=1'),  # normal input is unmodified
                     ('    ', '    '),  # blank lines are kept intact
                     ],
         ...
         # System calls
         escaped_shell=[(i, py3compat.u_format(o)) for i, o in \
         ...
         # Help/object info
         escaped_help=[(i, py3compat.u_format(o)) for i, o in \
                       [(u'?', 'get_ipython().show_usage()'),
                        ...
                        (u'?%%hist2',
                         "get_ipython().run_line_magic('pinfo', '%%hist2')"),
                        (u'?abc = qwe',
                         "get_ipython().run_line_magic('pinfo', 'abc')"),
                        ]],

         end_help=[(i, py3compat.u_format(o)) for i, o in \
                   [(u'x3?', "get_ipython().run_line_magic('pinfo', 'x3')"),
                    ...
                    (u'ax.*aspe*?',
                     "get_ipython().run_line_magic('psearch', 'ax.*aspe*')"),
         ...
         escaped_magic=[(i, py3compat.u_format(o)) for i, o in \
                        [(u'%cd', "get_ipython().run_line_magic('cd', '')"),
                         (u'%cd /home',
                          "get_ipython().run_line_magic('cd', '/home')"),
                         # Backslashes need to be escaped.
                         (u'%cd C:\\User',
                          "get_ipython().run_line_magic('cd', 'C:\\\\User')"),
                         (u'    %magic',
         ...
         mixed=[(i, py3compat.u_format(o)) for i, o in \
                [(u'In [1]: %lsmagic', "get_ipython().run_line_magic('lsmagic', '')"),
                 ...
                 (u'In [4]: b = %who',
                  "b = get_ipython().run_line_magic('who', '')"),
                 ]],
         )
...
syntax_ml = \
    dict(classic_prompt=[[('>>> for i in range(10):', 'for i in range(10):'),
                          ('...     print i', '    print i'),
                          ('... ', ''),
                          ],
                         [('>>> a="""', 'a="""'),
         ...
         ipy_prompt=[[('In [24]: for i in range(10):', 'for i in range(10):'),
                      ('   ....:     print i', '    print i'),
                      ('   ....: ', ''),
                      ],
                     ...
                     [('In [24]: for i in range(10):', 'for i in range(10):'),
                      # Space after last continuation prompt has been removed
                      # (issue #6674)
                      ('...:     print i', '    print i'),
                      ('...:', ''),
                      ],
         ...
         multiline_datastructure_prompt=[[('>>> a = [1,', 'a = [1,'),
                                          ('... 2]', '2]'),
                                          ],
                                         ],

         multiline_datastructure=[[('b = ("%s"', None),
                                   ('# comment', None),
                                   ('%foo )', 'b = ("%s"\n# comment\n%foo )'),
                                   ],
                                  ],

         multiline_string=[[("'''foo?", None),
                            ("bar'''", "'''foo?\nbar'''"),
                            ],
                           ],

         leading_indent=[[('    print "hi"', 'print "hi"'),
                          ],
...
def test_ipy_prompt():
    ...
    # Check that we don't transform the second line if we're inside a cell
    # magic
    transform_checker([
        (u'%%foo', '%%foo'),
        (u'In [1]: bar', 'In [1]: bar'),
