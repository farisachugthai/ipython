...
class Struct(dict):
    ...
    * Overloaded operators.
    ...
    Parameters
    ----------
    Can be initialized the same way that any `dict` is. See ``__init__`` as well.

    Attributes
    ----------
    _allownew : True
        Whether to allow calls to ``__setitem__``.
    ...
    def merge(self, __loc_data__=None, __conflict_solve=None, **kw):
        ...
        def preserve(old, new):
            ...
            Two argument functions which return the value that will go in the
            new struct.
